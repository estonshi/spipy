...
double maximize() {
	...
	#pragma omp parallel default(shared) private(r,d,t,x)
	{
		...
		if (omp_rank == 0) {
			...
			/*
			if (rank == 0 && (iteration%5 == 0 || iteration == 1)) {
				char fnameprob[999] ;
				sprintf(fnameprob, "%s/orientations/orientations_%.3d.bin", output_folder, iteration) ;
				int i=0;
				...
				rdata* rsort = (rdata*)malloc(num_rot_p * sizeof(rdata)) ;
				rdata buffer;
				FILE *fpprob = fopen(fnameprob, "w+") ;
				for(i=0;i<tot_num_data;i++){
					int j=0;
					for(j=0;j<num_rot_p;j++){
						rdata temp;
						temp.data = probab[j][i];
						temp.index = j;
						rsort[j] = temp;
					}
					qsort(rsort,tot_num_data,sizeof(rdata),cmp);
					for(j=0;j<=10;j++){
						buffer = rsort[j];
						//fwrite(&buffer, sizeof(double), 1, fpprob);
						fprintf(fpprob,"%f,%d\n",buffer.data,buffer.index);
					}
				}
				fclose(fpprob) ;
				//fwrite(probab, sizeof(double), sizeof(probab), fpprob) ;
			}
			*/
		}
		#pragma omp barrier
...
//					priv_likelihood[d_counter+d] += prob[d_counter+d] * (temp - sum_fact[d_counter+d] + count[d_counter+d]*log(scale[d_counter+d])) ;
		...
		#pragma omp barrier
		// before free probab, output orientations and probablities
			if (omp_rank==0 && rank == 0 && (iteration%10 == 0 || iteration == 1)) {
				fprintf(stderr, "Tomograms Updated.\n");
				char fnameprob[999] ;
				sprintf(fnameprob, "%s/orientations/orientations_%.3d.bin", output_folder, iteration) ;
				int i=0;
				typedef struct rindex{
				...
				for(i=0;i<tot_num_data;i++){
					int j=0;
					for(j=0;j<num_rot_p;j++){
						rsort[j].data = probab[j][i];
						rsort[j].index = j * num_proc + rank;
					} 
					//fprintf(stderr,"%f. Error under here.\n",rsort[0].data);
					qsort(rsort,num_rot_p,sizeof(rsort[0]),cmp);
					for(j=0;j<10;j++){
						buffer = rsort[j];
						//fwrite(&buffer, sizeof(double), 1, fpprob);
						fprintf(fpprob,"%f,%d \n",buffer.data,buffer.index);
